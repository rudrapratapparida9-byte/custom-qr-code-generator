...
(function () {
  ...
  function initQrEngine() {
    if (typeof QRCodeStyling === 'undefined') {
      console.warn('QRCodeStyling loading...');
      setTimeout(initQrEngine, 200);
      return;
    }
  ...
  function generatePayloadData() {
    switch (currentMode) {
      case 'url':
        return document.getElementById('urlInput').value.trim() || 'https://smartcalcpro.online';
  ...
  function setupTypeSelector() {
    typeBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        modeFields.forEach(f => f.classList.toggle('active', f.id === `mode-${currentMode}`));
        updateQrCode();
      });
    });
  }
  ...
  async function downloadQR(ext) {
    if (!qrCode) return;
    const res = parseInt(resolutionSelect.value, 10) || 512;
